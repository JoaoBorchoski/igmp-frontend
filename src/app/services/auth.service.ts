...
export class AuthService {
	...
	constructor(private restService: RestService, private router: Router, private poDialogService: PoDialogService) {
		this.userSubject = new BehaviorSubject<User>(JSON.parse(localStorage.getItem("@igmp:user")))
		this.user = this.userSubject.asObservable()
	}

	public get userValue(): User {
		const user = JSON.parse(localStorage.getItem("@igmp:user"))
		return user
	}

	public updateUser(user: User) {
		localStorage.removeItem("@igmp:user")
		localStorage.setItem("@igmp:user", JSON.stringify(user))
	}
			...
			this.restService.post("/sessions", event).subscribe({
				next: (response: ResponseProps) => {
					...
					if (!response.user.tfa || response.user.mustActiveTwoFactorAuthentication) {
						localStorage.setItem("@igmp:user", JSON.stringify(valuesToSave))
					}
              ...
              <p>
                <b>Motivo de bloqueio:</b> ${
					response.user.blockReasonId ? response.user.blockReasonId.description : "Não definido"
				}
              </p>
              <p>
                <b>Instruções de Reset:</b> ${
					response.user.blockReasonId
						? response.user.blockReasonId.instructionsToSolve
						: "Contate um administrador."
				}
              </p>`,
						})
						localStorage.removeItem("@igmp:user")
						localStorage.removeItem("@igmp:temp")
						reject()
					}
	...
	signOut() {
		localStorage.removeItem("@igmp:user")
		localStorage.removeItem("@igmp:temp")
		localStorage.removeItem("@igmp:preferences")
	...
	get tablePreferences() {
		return JSON.parse(localStorage.getItem("@igmp:preferences"))
	}
	...
	browseTablePreferences(route: string, preferences: any) {
		...
		localStorage.removeItem("@igmp:preferences")
		localStorage.setItem("@igmp:preferences", JSON.stringify(tablePreferences))
	}
}
