...
export class PedidoItemEditComponent implements OnInit, OnDestroy {
  ...
  getLiterals() {
    this.languagesService.getLiterals({ type: 'edit', module: 'operacao', options: 'pedidoItem'})
      .subscribe({
    ...
    if (this.pedidoItemForm.valid) {
      if (this.id && this.getPageType(this.activatedRoute.snapshot.routeConfig.path) === 'edit') {
        this.subscriptions.add(
          this.restService
            .put(`/pedidos-items/${this.id}`, data)
            .subscribe({
              next: () => {
