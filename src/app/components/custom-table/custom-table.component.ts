...
export class CustomTableComponent implements OnInit, OnDestroy {
	...
	@Input() removeMultiDeleteAction?: boolean = false
		...
		if (this.permissions.permitAll) {
			this.pageActions = [
				...
				},
				{
					...
					visible: !this.removeMultiDeleteAction,
				},
				{
	...
	search(search?: string) {
		...
		this.subscriptions.add(
			this.httpClient
				.post(
					this.getRoute(),
					this.getPayload({ page: 1, pageSize: this.pageSize, search, filter: this.filterExpression })
				)
				.pipe(
					finalize(() => {
						this.loading = false
